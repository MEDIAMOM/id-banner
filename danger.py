...
def major_login(access_token, open_id, platform_type=4, client_version="1.120.1"):
...
def get_login_data_with_jwt(jwt_token, ob_version, lock_region=None, enc_data=None):
    ...
    for attempt in range(MAX_RETRIES):
        try:
            if enc_data is None:
                dummy_payload = SimpleProtobuf.create_login_payload("dummy", "dummy", "4", "1.120.1", ob_version)
...
def perform_ban(eat=None, access=None, uid=None, password=None, token=None,
                client_version="1.120.1", ob_version="OB52"):
    """
...
def ban():
    try:
        ...
        client_version = request.args.get('client_version', '1.120.1')
        ob_version = request.args.get('ob_version', 'OB52')
